...
class TestCircuitBreakerBackoff(_StateFileMixin, unittest.TestCase):
    ...
    def test_escalating_backoff(self):
        # Doubles 15→30→60→120→240; the 6 h (360 min) cap holds only for the first
        # four failures. At the 5th consecutive failure the deep-failure ceiling lifts to
        # 24 h (1440 min), so the exponential keeps climbing (480, 960) and then pins at
        # 1440 — a sustained streak means a human must re-auth to clear it.
