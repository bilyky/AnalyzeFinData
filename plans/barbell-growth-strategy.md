...
* **The Rule:** The system must strictly enforce the `failure_dna_rules.json` feedback loop. Any asset carrying a Bearish (`Be` or `Be-`) PGR rating is treated as high-risk and is vetoed for the Spear bucket, even on an otherwise-qualifying momentum or breakout setup. This is a **deliberate Spear-only override** of the baseline elite-breakout waiver: on `main`, `check_failure_rules` delegates to `risk_utils.is_elite_breakout_candidate` (R&D #13 / PR #27), which *waives* the failure rules for elite high-score breakouts — the Spear intentionally does **not** honor that bypass and holds the no-waiver veto. 
* **The Rationale:** Buying momentum on fundamentally broken assets invites exposure to "dead cat bounces" and violent gap-downs. Furthermore, the true mathematical defense against an overnight gap is **Defined Risk via options** — buying a call (or a defined-risk spread) bounds the absolute maximum loss at the premium paid, sharply reducing (though never fully eliminating) pre-market gap exposure. Note that **leveraged/inverse ETFs are *not* defined-risk**: a −3× product *amplifies* an overnight gap rather than bounding it, and per the CLAUDE.md Aug-8 mandate `instruments.is_excluded()` already routes leveraged/inverse/crypto ETFs *out of* new long entries (they fall back to a generic ATR stop) until their level model is re-validated. Options are the gap defense here; leveraged ETFs are not.
