...
*   **High-Score PGR Bypass & R:R Waiver (R&D #13 & #32 Unification):** We delegate candidate validation to a centralized, AI-agnostic quantitative validation gate `is_elite_breakout_candidate()`. If a candidate exhibits an extreme, high-conviction Combined Trend Score ($\ge 8.0$) and positive Short10 momentum ($\ge 2.0$), the system completely waives both the Chaikin PGR rating constraint and the strict 2:1 Risk-to-Reward gate! This prevents slow fundamental data or static R:R traps from blocking high-alpha breakouts (such as `SAM` or `CVEO`).
*   **Split-Adjusted Level Detection (`risk_utils._split_adjust_ohlcv`):** The local OHLCV cache is raw Alpha-Vantage, **unadjusted** (no adjusted-close field), so a stock split inside the pivot/ATR lookback leaves pre-split bars at ¼× or 4× the current price scale — poisoning swing-low support, swing-high resistance, and ATR. Every unadjusted series is now back-adjusted onto the current scale (cumulative product of detected split ratios) before `detect_support`/`detect_resistance`/`_atr_from_series` and the separate `calculate_atr` path run. A day-over-day close ratio outside `[0.55, 1.8]` is a split candidate, confirmed **only** when the open moved by ~the same factor (`|r_open − r_close| ≤ 0.35·r_close`) — so a real intraday crash (open near prior close) is preserved, not adjusted away. No-split symbols hit a fast path with zero behavior change. Surfaced by the Aug-8 leveraged/inverse/crypto levels study, where unadjusted pivots were the dominant cause of that cohort's low backtest win-rate.
...
*   **Symbology Cross-Referencing & US Format Filtration (`verify_symbols`):** Validates extracted symbols against your active (~485-symbol) Research universe, automatically attaching the Power Gauge Rating (PGR) and combined technical score to matching symbols, and isolating unlisted plays. Employs a strict **`is_valid_us_ticker()`** regex gate to automatically filter out non-tradeable garbage tokens, private companies, or foreign suffixes (such as `SpaceX`, `Anthropic`, or `2449.TW`) from your reports.
...
*   **Daily-Repeating Task Triggers (`register_agent_tasks.ps1`):** Registers Windows Task Scheduler triggers with `-Daily` repetition so the Watchdog and Stop Monitor continue through weekends without manual re-registration. All active scheduled tasks run headlessly as **S4U background services (No-Password `/np` mode)** under your Windows user account, permanently preventing Event ID 332 "user not logged on" scheduling failures.
...
*   **The \"AETHER Healer\" (`watchdog.py`):** Headless, synchronously blocking AI self-healing loop. Intercepts pipeline tracebacks, invokes `claude.exe` (configured via `AETHER_HEALER_CMD` env var) with a structured 6-step diagnostic protocol (`prompts/self_healing.md`), applies a minimal code fix, runs the full test suite, and commits only on green. A `self_healing.lock` circuit breaker prevents recursive loops.
    *   *Cross-Process Execution Singleton (`watchdog_run.lock`):* Implements a strict, atomic file-lock (`Data/watchdog_run.lock` with native `tasklist` PID matching and automatic `atexit` cleanup handlers) that immediately exits if another instance is already running, preventing dual-process file locks, database conflicts, or log collusions.
    *   *Non-Destructive Network Synchronization & Task Auto-Purging:* 
        *   *Network Sync:* Recursively synchronizes and backs up your local `Data/` folder to your UNC network storage (`\\\\10.0.0.156\\Storage\\`) using native `robocopy` with non-destructive `/E` parameters. Automatically skips/pauses during active NYSE trading hours (6:30 AM - 1:15 PM PST) to prevent active file-locking conflicts.
        *   *Stray-Task Auto-Purging (Pillar 1 Self-Sanitation):* Scans Windows Task Scheduler hourly, parses all AETHER/AnalyzeFinData tasks, and programmatically exports stray, non-whitelisted, or duplicate tasks (such as `AnalyzeFinData_Morning_Test`) to `Data/task_backups/` as an XML config, deletes them, and dispatches an active email warning notification.
...
*   **Proactive E*TRADE Session Keeper & Anti-Ban Cooldown (`watchdog.py` & `aether/etrade.py`):**
    *   *Hourly renewals:* The watchdog silently executes E*TRADE access token renewals every 1 hour in the background. The renewal lockout threshold is set to **`55 minutes`** so that hourly tasks can successfully renew the session without drifting into soft-expiry.
    *   *Pre-Flight Verification:* Actively verifies cached tokens against E*TRADE's market clock before return. If unauthorized, invalidates cache and triggers Playwright headless re-auth inline.
    *   *15-Minute Reauth Cooldown (Anti-Ban Guard):* Implements a strict 15-minute reauth cooldown (`etrade_cooldown.lock`) inside `get_tokens()` to prevent rapid-fire browser logins during connection glitches, permanently shielding your brokerage account from rate-limiting bans.
    *   *Zero-Touch Headless Daily Mint via Software TOTP (`aether/etrade/__init__.py::_get_verifier_via_totp`):* When an `etrade_totp_secret` (Symantec VIP SYMZ) is configured, the cold daily reauth self-completes MFA with a `pyotp`-generated 6-digit code typed into E*TRADE's dedicated `#securityCode` field (never appended to the password) — no SMS wall, so the overnight door needs no human. A `_fill_verified()` guard types-then-verifies each field to survive the login form's skeleton→hydrate race (dropped keystrokes → recovers via `fill()`), and the same anti-ban circuit breaker still gates every attempt. Wired through the one `_login_headless` choke point; a TOTP-configured env is treated as trusted so an unseeded profile no longer blocks the mint. Provisioning + one-time E*TRADE 2FA registration is done via `scripts/etrade_totp_wizard.py`. Offline-hermetic tests in `tests/test_etrade_totp_login.py`; the daily task is only installed after one live headed mint passes on the clean-egress host. The browser engine is selectable via the `etrade.login_strategy` config knob (env `ETRADE_LOGIN_STRATEGY`) resolved by `_resolve_login_strategy()` — `auto` (default; Firefox+TOTP when a secret is set, else the Chromium device-trust profile), `firefox_totp` (throwaway Firefox; requires a secret, else logs+falls back), or `persistent_profile` (Chromium profile that self-fills the security code when a secret is present) — because which engine clears Akamai depends on the run's network egress (residential/clean vs corporate/VPN); the same resolver now also drives the interactive `get_tokens` verify so a supervised login self-completes MFA instead of stalling on the historic corporate-egress spinner-hang.
...
*   **Dynamic Pyramiding (Sizing Up Winners) & Pullback Dip Scale-In (`ai_portfolio_game.py`):** If cash exceeds `10.0%` of equity, the system dynamically scales into active, high-performing green positions. It allows pyramiding scale-ins on profitable, risk-locked winners that have strong long-term trend support (`l60 >= 2.0`) even during short-term pullback dips (`s10 >= 0.0`), purchasing more shares (up to the 15% maximum position allocation cap) and cleanly recalculating their blended cost bases.
