...
*   **The \"AETHER Healer\" (`watchdog.py`):** Headless, synchronously blocking AI self-healing loop. Intercepts pipeline tracebacks, invokes `claude.exe` (configured via `AETHER_HEALER_CMD` env var) with a structured 6-step diagnostic protocol (`prompts/self_healing.md`), applies a minimal code fix, runs the full test suite, and commits only on green. A `self_healing.lock` circuit breaker prevents recursive loops.
    ...
    *   *Non-Destructive Network Synchronization:* Recursively synchronizes and backs up your local `Data/` folder to your UNC network storage (`\\\\10.0.0.156\\Storage\\`) using native `robocopy` with non-destructive `/E` parameters (overwriting newer/updated files but never deleting any historical items in your backup folder). Automatically **skips and pauses** during active NYSE trading hours (6:30 AM - 1:15 PM PST, weekdays) to completely protect your live database, spreadsheet, and monitor files from file-locking conflicts.
