...
sys.stdout.write(f"Testing {len(candidates)} symbols  date={DATE.date()}  OHLCV lookback={LOOKBACK}d\n\n")
sys.stdout.write(f"{'Symbol':<8} {'Price':>8} {'NFiles':>7} {'Chain':>9} {'OHLCV':>9} {'Diff%':>7} {'ChainMs':>8} {'OhlcvMs':>7}\n")
sys.stdout.write("-" * 72 + "\n")
...
for n_files, sym in candidates:
    ...
    sys.stdout.write(f"{sym:<8} {pg.price:>8.2f} {n_files:>7} {chain_str} {ohlcv_target:>9.2f} {diff_str} {chain_ms:>7.1f}ms {ohlcv_ms:>6.1f}ms\n")

sys.stdout.write("-" * 72 + "\n")
sys.stdout.write(f"Chain vs OHLCV compared for {compared}/{len(candidates)} symbols\n")
sys.stdout.write(f"Chain total {total_chain_ms:.0f}ms  OHLCV total {total_ohlcv_ms:.0f}ms\n")
